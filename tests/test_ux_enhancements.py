
from dataclasses import dataclass, field
from unittest.mock import MagicMock, patch

import pytest

from parsfet.models.liberty import Cell, LibertyLibrary
from parsfet.reporting.html_generator import generate_report

# --- Minimal Mocks for Dependencies ---

@dataclass
class MockMetrics:
    cell_type: str = "inverter"
    def to_dict(self):
        return {"area_ratio": 1.0, "d0_ratio": 1.0, "p0_ratio": 1.0}

@dataclass
class MockBaseline:
    cell_name: str = "INV_X1"
    fo4_slew: float = 0.01

@dataclass
class MockNormalizer:
    baseline: MockBaseline = field(default_factory=MockBaseline)
    fo4_load: float = 0.005
    baseline_cell: MagicMock = field(default_factory=MagicMock)

    def normalize(self, cell):
        return MockMetrics()

    # Mocking unit conversion methods
    def normalize_time(self, val): return val
    def normalize_capacitance(self, val): return val

@dataclass
class MockDatasetEntry:
    library: LibertyLibrary
    normalizer: MockNormalizer

@pytest.fixture
def mock_library():
    lib = LibertyLibrary(
        name="test_lib",
        technology="cmos",
        delay_model="table_lookup",
        time_unit="1ns",
        voltage_unit="1V",
        current_unit="1mA",
        leakage_power_unit="1nW",
        pulling_resistance_unit="1kohm",
        capacitive_load_unit=(1.0, "pf"),
        cells={}
    )
    return lib

@pytest.fixture
def mock_cell():
    cell = MagicMock(spec=Cell)
    cell.name = "INV_X1"
    cell.area = 1.0
    cell.linear_delay_model.return_value = (0.1, 0.5, 0.99)
    cell.linear_power_model.return_value = (0.01, 0.2, 0.95)
    cell.timing_arcs = []
    cell.power_arcs = []
    return cell

@patch("parsfet.reporting.html_generator.validate_assets")
def test_ux_features_presence(mock_validate, tmp_path, mock_library, mock_cell):
    """Verify that UX enhancements ARE present in the generated report."""
    output_file = tmp_path / "report_ux.html"

    mock_library.cells["INV_X1"] = mock_cell
    entry = MockDatasetEntry(library=mock_library, normalizer=MockNormalizer())

    generate_report([entry], output_file)

    content = output_file.read_text("utf-8")

    # 1. Check for global keyboard handler
    assert '@keydown.window="handleKeydown($event)"' in content
    assert 'handleKeydown(e) {' in content

    # 2. Check for visual hint in search
    assert 'placeholder="Filter cells... (Press /)"' in content

    # 3. Check for visual hints on tabs
    # We look for the "1" and "2" badges
    assert '>1</span>' in content
    assert '>2</span>' in content
