...
This module handles the generation of interactive HTML reports for visualizing
library metrics, model fits, and comparisons. It prepares the data by extracting
interpolation curves from the Liberty tables and injecting them into a Jinja2-like
HTML template.
"""
...
def interpolate_1d_at_slew(
    table: Optional[LookupTable], target_slew: float
) -> tuple[Optional[list[float]], Optional[list[float]]]:
    """Interpolates a 2D table to get a 1D curve (Load vs Delay) at a fixed slew.

    Extracts a slice of the table surface at `index_1 = target_slew`.

    Args:
        table: The 2D LookupTable to interpolate.
        target_slew: The input slew value (index_1) to slice at.

    Returns:
        A tuple (loads_list, values_list) where:
        - loads_list: The x-axis values (output load / index_2).
        - values_list: The y-axis values (delay or power).
        Returns (None, None) if the table is invalid or empty.
    """
...
def interpolate_1d_at_load(
    table: Optional[LookupTable], target_load: float
) -> tuple[Optional[list[float]], Optional[list[float]]]:
    """Interpolates a 2D table to get Output Slew vs Input Slew at fixed Load.

    Extracts a slice of the table surface at `index_2 = target_load`.
    Used for generating convergence plots (S_out vs S_in).

    Args:
        table: The 2D LookupTable to interpolate.
        target_load: The output load value (index_2) to slice at.

    Returns:
        A tuple (input_slews_list, output_slews_list) where:
        - input_slews_list: The x-axis values (input slew / index_1).
        - output_slews_list: The y-axis values (output slew).
        Returns (None, None) if the table is invalid or empty.
    """
    if not table or not table.values:
        return None, None
...
def generate_report(entries: list[Any], output_path: Path):
    ...
    Prepares a JSON payload containing all necessary plot data (scatter points
    and linear fit lines) and injects it into the HTML template.

    Args:
        entries: List of DatasetEntry objects (from parsfet.data).
        output_path: Path to write the generated HTML file.

    Raises:
        FileNotFoundError: If the report template file is missing.
    """
    ...
    for entry in entries:
        ...
        for cell_name, cell in library.cells.items():
            ...
            # Model Line (Delay)
            if max_load == 0:
                max_load = 0.1  # Default if no arcs
            x_model = [0, max_load * 1.1]
            y_model = [d0 + k_d * x for x in x_model]

            delay_plot = {"scatters": delay_scatters, "model": {"x": x_model, "y": y_model}}
            ...
            # Model Line (Power)
            if max_load_p == 0:
                max_load_p = max_load  # usage fallback
            x_model_p = [0, max_load_p * 1.1]
            y_model_p = [e0 + k_p * x for x in x_model_p]

            power_plot = {"scatters": power_scatters, "model": {"x": x_model_p, "y": y_model_p}}

            # Combine
            cells_data.append(
                {
                    "name": cell_name,
                    "type": metrics.cell_type.name,  # Use enum name directly
                    "delay_r2": r2_d,
                    "power_r2": r2_p,
                    "metrics": metrics_dict,  # Full details for the JSON viewer
                    "plots": {"delay": delay_plot, "power": power_plot},
                }
            )
        ...
        if normalizer.baseline_cell and hasattr(normalizer.baseline_cell, "timing_arcs"):
            ...
            if target_arc:
                ...
                if target_arc.rise_transition:
                    rise_in, rise_out = interpolate_1d_at_load(
                        target_arc.rise_transition, raw_fo4_load
                    )

                # Get Fall Curve (if exists)
                fall_in, fall_out = [], []
                if target_arc.fall_transition:
                    fall_in, fall_out = interpolate_1d_at_load(
                        target_arc.fall_transition, raw_fo4_load
                    )
                ...
                baseline_lut = {
                    ...
                    "convergence": {"input_slew": conv_in, "output_slew": conv_out},
                }
    ...
    final_html = template_content.replace("{{ lib_data_json }}", json_str)
